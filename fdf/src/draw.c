...
/*   Updated: 2025/01/18 11:46:45 by William          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/fdf.h"

static void	draw_pixel(t_image image, t_crd point, int color)
{
	char	*pixel;

	if (!is_pixel_out_of_bounds(point))
	{
		pixel = image.addr + (point.y
				* image.line_length
				+ point.x
		...
		*(int *)pixel = color;
		...
		ft_printf("INFO: pixel (%i, %i) out of bounds...\n", point.x, point.y);
}
...
void	draw_line(t_image image, t_crd src, t_crd dest, t_primitive z)
{
	...
	while (1)
	{
		draw_pixel(image, line.src, get_colour(line.current_z, z.a, z.b));
